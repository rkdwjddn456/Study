...
def extract_frame(video_path, process_annotations, file_name, train_f, val_f, test_f):
        ...
        while True:
            ...
            if ret:
                ...
                for process_annotation in split_process_annotations:
                    ...
                    if count % 16 == 0:
                        cv2.imwrite('val_images/' + jpg_file_name, crop_img)
                        val_f.write(dataset_line)
                    elif count % 17 == 0:
                        cv2.imwrite('test_images/' + jpg_file_name, crop_img)
                        test_f.write(dataset_line)
                    elif count % 2 == 0:
                        cv2.imwrite('train_images/' + jpg_file_name, crop_img)
...
def processing(video_dir, train_f, val_f, test_f):
    for root, dirs, files in os.walk(video_dir):
        for file in files:
            ...
            if extension == ".mp4":
                ...
                extract_frame(video_path, process_annotations, file_name, train_f, val_f, test_f)

if __name__ == "__main__":
    createFolder("./train_images")
    createFolder("./val_images")
    createFolder("./test_images")

    video_dir = "../sample_video/video"

    with open("./train_dataset.txt", 'w', encoding='utf8') as train_f:
        with open("./val_dataset.txt", 'w', encoding='utf8') as val_f:
            with open("./test_dataset.txt", 'w', encoding='utf8') as test_f:
                processing(video_dir, train_f, val_f, test_f)