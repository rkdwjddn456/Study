...
class Thread1(QThread):
    ...
    def run(self):
        ...
        res = requests.post(f'http://192.168.25.44:5000/fileupload_QT/{now}', files= upload)
...
class Thread2(QThread):
    ...
    def run(self):
        global running, count, start_frame_number, video_length
...
class MainWidget(QWidget):
    def __init__(self):
        ...
        btn7 = QPushButton("파일 받기")
        btn7.clicked.connect(self.file_receive)

        btn8 = QPushButton("사진 보기")
        btn8.clicked.connect(self.show_images)
    ...
    def file_receive(self):
        global video_length, now
        break_point = (video_length / 20) + 2
        break_point = int(break_point)
        for i in range(1, break_point + 1):
            url = "http://192.168.25.44:5000/static/qt/"+now+"/output_images/" +"%05d" %i+ ".jpg"
            parse_file = urlparse(url)
            file_name = os.path.basename(parse_file.path)
            file = requests.get(url)
            open(file_dir+"/images/"+file_name, 'wb').write(file.content)
        print("파일 받기 완료")
    
    def show_images(self):
        iname = QFileDialog.getOpenFileName(self)
...
if __name__ == '__main__':
    file_dir = r"C:\Users\Administrator\Desktop\docker_v"
    running = False
